...
o_clumped <- o %>%
    mutate(pval=10^-LP) %>%
    ieugwasr::ld_clump(., bfile="../data/reference/ld/EUR", plink_bin=genetics.binaRies::get_plink_binary())
o_clumped
```
...
prs <- generate_prs(vcf, bfile="../data/reference/ld/EUR", plink_bin=genetics.binaRies::get_plink_binary())
...
```{r}
create_pval_index_from_vcf(vcfurl, pval=0.05, indexname="index.pvali")
